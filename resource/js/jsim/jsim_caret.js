...
//		element = JS_IM_Common.$(element);
		...
		// }
		return tinymce.activeEditor.selection.getRng().startOffset;
	},
...
//		element = JS_IM_Common.$(element);
		...
		return new Array(selectionStart, selectionEnd - selectionStart);
	},
...
//		element = JS_IM_Common.$(element);
		...
		// 	var click_e=str.substr(selected_range[0] + selected_range[1], $(element).text().length);
		// 	$(element).text( click_s + word + click_e );
		...
		//if ( txt.substring(rng.startOffset, rng.startOffset + 1) == '' ) {
			// var len = word.length ? word.length : txt.length;
			// console.log('startOffset:'+rng.startOffset);
...
//		element = JS_IM_Common.$(element);
		element.focus();
		console.log("setCaretPosPurWord: "+pos+','+s);
...
//		element = JS_IM_Common.$(element);
	...
	getCaretXY:function(element,repaint){
		if(!tinymce.activeEditor) return new Array(0,0);
		var curPos = tinymce.activeEditor.selection.getStart();
		var editorPos = $('#fixed-editor iframe').offset();
		return new Array(editorPos.left + elem.offsetLeft, editorPos.top + elem.offsetTop);
